...
class SegmentProcessor:
    def __init__(
        self,
        pdf_reading_order_tokens_list: list[PdfReadingOrderTokens],
        segment_types: list[TokenType] = None,
    ...
    def process_a_paragraph(self, paragraph: Paragraph, pdf_reading_order_tokens: PdfReadingOrderTokens):
        ...
        if self.segment_types is not None and segment_type not in self.segment_types:
            return
        page = pdf_reading_order_tokens.pdf_features.pages[paragraph.tokens[0].page_number - 1]
        label_page = (
            pdf_reading_order_tokens.labeled_page_by_raw_page[page]
            if page in pdf_reading_order_tokens.labeled_page_by_raw_page
            else None
        )
        segment_token = self.get_processed_token_from_paragraph(paragraph.tokens, label_page)
        self.add_processed_token_to_page(segment_token, paragraph.tokens, page)

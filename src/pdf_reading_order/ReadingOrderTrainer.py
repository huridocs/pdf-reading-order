...
class ReadingOrderTrainer(PdfTrainer):
    def get_candidate_tokens_for_current_token(self, current_token: PdfToken, possible_candidate_tokens: list[PdfToken]):
        features = [
            ReadingOrderCandidatesTrainer.get_candidate_token_features(current_token, candidate_token)
    ...
    def loop_candidate_tokens_in_page(self, page: PdfPage):
        ...
        for current_token in page_tokens:
            possible_candidate_tokens = [
                token for token in page_tokens if token != current_token and token.id != "pad_token"
            ]
        ...
        for current_token, candidate_token_1, candidate_token_2, token_features, page in list(self.loop_candidate_tokens()):
            features_rows.append(
                self.get_reading_order_features(
                    current_token, candidate_token_1, candidate_token_2, token_features, page.tokens
                )
            )
    ...
    def get_next_token(
        ...
        token_features: TokenFeatures,
        page: PdfPage,
        remaining_tokens: list[PdfToken],
        current_token: PdfToken,
    ):
        ...
        candidate_tokens = self.get_candidate_tokens_for_current_token(current_token, possible_candidate_tokens)
        next_token = self.find_next_token_from_candidates(
